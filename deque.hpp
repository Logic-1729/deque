...
size_t lower_block_size = block_size >> 1;
size_t upper_block_size = block_size;
...
private:
    ...
    void check() {
        ...
        while (tmp->nxt != tail){
            ...
            if (tmp->length + tmp->nxt->length <= lower_block_size) {
                merge(tmp, tmp->nxt);
...
public:
    iterator insert(iterator pos, const T &value) {
        ...
        if (pos.cur_block->length >= upper_block_size) {
            list_node* tmp_node = pos.cur_block->head;
            int tmp = 0;
            while (tmp < lower_block_size) {
                tmp_node = tmp_node->nxt;
                ++tmp;
            }
            split(tmp_node, pos.cur_block);
            if (pos.block_index < lower_block_size) { return iterator(this, new_node, pos.block_index, pos.cur_block);} 
            else {return iterator(this, new_node, pos.block_index - lower_block_size, pos.cur_block->nxt);}
    ...
    void pop_back() {
        ...
        if (pos.block_index == 0) {
            ...
            pos.cur_node->nxt->pre = pos.cur_node->pre;
            pos.cur_block->length--;
            if (pos.cur_block->pre != head && pos.cur_block->pre->length + pos.cur_block->length <= lower_block_size) {
    ...
    void pop_front() {
        ...
        if (pos.cur_block->length == 0 && pos.cur_block->nxt != tail) {
            ...
            if (pos.cur_block->length + pos.cur_block->nxt->length <= lower_block_size)
